...
def collect_trajectories(env, policy, n_episodes=10):
    env = SubprocVecEnv([make_env_Multi(env_name, seed) for seed in range(nproc)], start_method = 'fork') #[make_env(env.unwrapped.spec.id)])
    # env.seed(SEED)
    expert_data = []
    expert_action = []
...
expert = train_expert(env_name, training_algo=training_algo, total_expert_timesteps=10_000_000)
